...
def get_message(top,page):
    ...
    sql = '''select * from myapp_xinli where  topic={} and id>{} and id <={} and "delete"=0'''.format( top, start, end)
    res = connect(sql)
    data_list = []
    for i in res:
        t = {
        ...
        data_list.append(t)
    sql2 = '''select count(*) from myapp_letter where topic={} and "delete"=0'''.format(
        top)
    res2 = connect(sql2)
    for i in res2:
        num = i[0]
